...
int main(){
    ...
    kmp(t, s, suffix);
    for(int i=(int)s.size()-1, j=(int)t.size()-1; i<t.size(); i++, j--) {
        if(prefix[i] + suffix[j] >= s.size()) {
            cout << "yes\n"; return 0;
        }
    }
    cout << "no\n";
...
// rolling hashes? im gonna roll your hashes
// ts pmo 🥀💔
